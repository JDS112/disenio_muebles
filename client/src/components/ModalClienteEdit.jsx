...
import {updateCliente} from "../api/clienteApi.js"
import { useContext, useEffect} from "react"
import { AppContext } from "../context/AppContext.jsx"
function ModalClienteEdit() {
    const {register, handleSubmit, setValue,reset} = useForm()
    const context = useContext(AppContext)
    const onSubmit = async data => {
        try {
            const res = await updateCliente(context.object.uid,data)
            console.log(data)
            console.log(res)
  ...
  return (
    <>
        ...
        <div className="relative bg-[rgb(30,31,34)] p-5 rounded-lg flex flex-col justify-center items-center gap-5 z-20 shadow-2xl">
            ...
            <h1 className="text-white text-2xl">Actualizar Cliente</h1>
